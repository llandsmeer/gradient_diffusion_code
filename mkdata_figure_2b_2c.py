...
def register():
    ...
    forward(GSoma, 'io_soma_bwd', guess)
    forward(GDend, 'io_dend_bwd', guess)
    forward(GAxon, 'io_axon_bwd', guess)
...
class Recipe(arbor.recipe):
    ...
    def probes(self, _): return [
            arbor.cable_probe_membrane_voltage_cell(),
                    ...
                    arbor.cable_probe_density_state_cell('io_soma_bwd', name) for name in GSoma.names
            ] + [
                    arbor.cable_probe_density_state_cell('io_dend_bwd', name) for name in GDend.names
            ] + [
                    arbor.cable_probe_density_state_cell('io_axon_bwd', name) for name in GAxon.names
            ]
...
names =  [
                    f'soma_{name}' for name in GSoma.names
            ] + [
                    f'dend_{name}' for name in GDend.names
            ] + [
                    f'axon_{name}' for name in GAxon.names
            ]
...
dsdthhandles = [sim.sample((0, 1+i), arbor.regular_schedule(1.))
...
out['v'] = data
out['v_meta'] = str(meta)
for handle, name in zip(dsdthhandles, names):
